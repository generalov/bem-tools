...
var Deps = exports.Deps = INHERIT({
    ...
    add: function(target, depsType, item) {
        ...
        // TODO: clone item without 'tech' and 'key'
        // TODO: add item into cloned item as shouldDeps
        // if (item.item.tech) {
        //     var baseItem = item.clone();
        //     delete baseItem.item.tech;
        //     delete baseItem.key;
        //     var baseItemKey = baseItem.buildKey();

        //     items[baseItemKey] && items[baseItemKey].shouldDeps.push(itemKey);
        // }
        // ---
    ...
    forEach: function(fn, uniq, itemsByOrder, ctx) {
        ...
        // var tmp = this.items[''].shouldDeps.filter(function(i) {
        //     return !_this.items[i].item.tech;
        // });

        // (itemsByOrder || tmp).forEach(function(i) {
    ...
    serializeExt: function() {
        var _this = this,
            deps = [];

        this.forEach(function(item, ctx) {
            item.shouldDeps.length && (item.item.shouldDeps = item.shouldDeps.map(function(key) {
                return _this.items[key].item;
            }));

            item.mustDeps.length && (item.item.mustDeps = item.mustDeps.map(function(key) {
                return _this.items[key].item;
            }));
    ...
    stringify: function() {
        var res = [],
            deps = this.serialize(),
            depsExt = this.serializeExt();

        // console.log(JSON.stringify(deps, null, 4));
        // console.log('-----');
        // console.log(JSON.stringify(depsExt, null, 4));
        ...
        U.isEmptyObject(deps) || res.push('exports.depsByTechs = ' + JSON.stringify(depsExt, null, 4) + ';\n');
